...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const processExtractedData = (isForSimulado: boolean = false) => {
    ...
    let motivationalTextId: string | undefined = undefined;
    if (extractedData.textos_motivadores && extractedData.textos_motivadores.length > 0) {
      extractedData.textos_motivadores.forEach((text: any) => {
        ...
        if (!motivationalTextId) motivationalTextId = newId; // Pega o ID do primeiro texto para o simulado
      });
    }

    const newQuestionIds: string[] = [];
    extractedData.questoes.forEach((q: any) => {
      const newQuestion: Question = {
        id: 'ai_' + Date.now() + Math.random(),
        enunciado: q.text, // Mapeamento do campo 'text' da IA para 'enunciado'
        options: q.options,
        ...
        // Adicionando os novos campos da arquitetura suprema
    ...
    if (isForSimulado) {
        const newSim: Simulado = {
          ...
          motivationalTextId: motivationalTextId,
        };
        onAddSimulado(newSim);
        alert(`Simulado "${simTitle}" criado com ${newQuestionIds.length} questões e texto de apoio!`);
    } else {
        alert(`${extractedData.questoes.length} questões foram salvas no banco com sucesso!`);
    }
